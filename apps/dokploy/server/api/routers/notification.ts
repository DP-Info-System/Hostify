...
export const notificationRouter = createTRPCRouter({
	...
	testTelegramConnection: withPermission("notification", "create")
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, "Hi, From Hostify 👋");
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", "Hi, From Hostify 👋"),
					color: 0xf3f7f4,
				});
	...
	receiveNotification: publicProcedure
		.input(
			z.object({
				ServerType: z.enum(["Hostify", "Remote"]).default("Hostify"),
		...
		.mutation(async ({ input }) => {
			try {
				await sendNtfyNotification(
					...
					"Test Notification",
					"",
					"view, visit Hostify on Github, https://github.com/hostify/hostify, clear=true;",
					"Hi, From Hostify 👋",
				);
				return true;
			} catch (error) {
		...
		.mutation(async ({ input }) => {
			try {
				await sendMattermostNotification(input, {
					text: "Hi, From Hostify 👋",
					channel: input.channel,
					username: input.username || "Hostify Bot",
				});
				return true;
			} catch (error) {
				throw new TRPCError({
		...
		.mutation(async ({ input }) => {
			try {
				await sendTeamsNotification(input, {
					title: "🤚 Test Notification",
					facts: [{ name: "Message", value: "Hi, From Hostify 👋" }],
				});
				return true;
			} catch (error) {
				throw new TRPCError({
