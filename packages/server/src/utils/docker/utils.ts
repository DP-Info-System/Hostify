 ...
 *
 * https://github.com/Hostify/hostify/pull/3064
 */
 ...
 *
 * https://github.com/Hostify/hostify/pull/3267
 */
...
export const generateConfigContainer = (
	...
	return {
		...
		...(networkSwarm
			...
			: {
					Networks: [{ Target: "hostify-network" }],
				}),
		...(endpointSpecSwarm && {
			EndpointSpec: {
...
export const checkPostgresHealth = async (): Promise<ServiceHealthStatus> => {
	const serviceCheck = await checkSwarmServiceRunning("hostify-postgres");
	if (serviceCheck.status === "unhealthy") {
		return serviceCheck;
	}

	// Verify PostgreSQL actually accepts connections
	const containerId = await getSwarmServiceContainerId("hostify-postgres");
	if (!containerId) {
...
export const checkRedisHealth = async (): Promise<ServiceHealthStatus> => {
	const serviceCheck = await checkSwarmServiceRunning("hostify-redis");
	if (serviceCheck.status === "unhealthy") {
		return serviceCheck;
	}

	// Verify Redis actually responds to PING
	const containerId = await getSwarmServiceContainerId("hostify-redis");
	if (!containerId) {
		return { status: "unhealthy", message: "Could not find running container" };
	}
...
export const checkTraefikHealth = async (): Promise<ServiceHealthStatus> => {
	...
	try {
		const container = docker.getContainer("hostify-traefik");
		...
		return checkSwarmServiceRunning("hostify-traefik");
	}
};
