...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "hostify-restore-"));
		...
		try {
			...
			const { stdout: postgresContainer } = await execAsync(
				`docker ps --filter "name=hostify-postgres" --filter "status=running" -q | head -n 1`,
			);

			if (!postgresContainer) {
				throw new Error("Hostify Postgres container not found");
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U hostify postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'hostify' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U hostify postgres -c "DROP DATABASE IF EXISTS hostify;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U hostify postgres -c "CREATE DATABASE hostify;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U hostify -d hostify /tmp/database.sql`,
			);
