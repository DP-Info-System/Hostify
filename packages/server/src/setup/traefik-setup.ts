...
export const initializeStandaloneTraefik = async ({
	...
	const containerName = "hostify-traefik";
	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				`${DYNAMIC_TRAEFIK_PATH}:/etc/hostify/traefik/dynamic`,
			...
			ContainerSpec: {
				...
				Mounts: [
					...
					},
					{
						...
						Target: "/etc/hostify/traefik/dynamic",
					},
					{
						Type: "bind",
...
export const createDefaultServerTraefikConfig = () => {
	...
	const configFilePath = path.join(DYNAMIC_TRAEFIK_PATH, "hostify.yml");
...
export const getDefaultTraefikConfig = () => {
	const configObject: MainTraefikConfig = {
		...
		providers: {
			...
			file: {
				directory: "/etc/hostify/traefik/dynamic",
				watch: true,
			},
		},
		entryPoints: {
			...
			certificatesResolvers: {
				letsencrypt: {
					acme: {
						...
						storage: "/etc/hostify/traefik/dynamic/acme.json",
						httpChallenge: {
							entryPoint: "web",
						},
					},
