...
/** Returns current Hostify docker image tag or `latest` by default. */
export const getHostifyImageTag = () => {
	return process.env.RELEASE_TAG || "latest";
};

/** Returns Hostify docker service image digest */
export const getServiceImageDigest = async () => {
	const { stdout } = await execAsync(
		"docker service inspect hostify --format '{{.Spec.TaskTemplate.ContainerSpec.Image}}'",
	);
...
export const getUpdateData = async (
	...
	try {
		const baseUrl =
			"https://hub.docker.com/v2/repositories/hostify/hostify/tags";
		...
		const currentImageTag = getHostifyImageTag();
...
export const reloadDockerResource = async (
	...
	if (resourceType === "service") {
		if (resourceName === "hostify") {
			...
			command = `docker service update --force --image hostify/hostify:${imageTag} ${resourceName}`;
		} else {
...
export const checkPortInUse = async (
	...
	try {
		...
		const dockerCommand = `docker ps -a --format '{{.Names}}' | grep -v '^hostify-traefik$' | while read name; do docker port "$name" 2>/dev/null | grep -q ':${port}' && echo "$name" && break; done || true`;
		...
		// Hostify runs inside a container, so we spawn an ephemeral container
...
export const reconnectServicesToTraefik = async (serverId?: string) => {
	...
	for (const compose of composeResult) {
		commands += `docker network connect ${compose.appName} $(docker ps --filter "name=hostify-traefik" -q) >/dev/null 2>&1\n`;
	}
